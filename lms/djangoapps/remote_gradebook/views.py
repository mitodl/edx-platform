...
def get_enrolled_non_staff_users(course):  # pylint: disable=deprecated-lambda
    """
...
def list_remote_students_in_section(request, course_id):
    ...
    course_id = CourseLocator.from_string(course_id)
    course = get_course_by_id(course_id)
    error_msg, datatable = get_remote_gradebook_datatable_resp(request.user, course, 'get-membership', section=section_name)
