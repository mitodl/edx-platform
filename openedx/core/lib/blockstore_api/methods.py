...
import blockstore.apps.api.exceptions as blockstore_api_exceptions
...
def toggle_blockstore_api(func):
    ...
    def wrapper(*args, **kwargs):
        if use_blockstore_app():
            ...
            # Convert the blockstore.apps.api exception into a blockstore_api exception
            except blockstore_api_exceptions.CollectionNotFound as exc:
                raise CollectionNotFound from exc
            except blockstore_api_exceptions.BundleNotFound as exc:
                raise BundleNotFound from exc
            except blockstore_api_exceptions.BundleVersionNotFound as exc:
                raise BundleVersionNotFound from exc
            except blockstore_api_exceptions.DraftNotFound as exc:
                raise DraftNotFound from exc
            except blockstore_api_exceptions.BundleFileNotFound as exc:
                raise BundleFileNotFound from exc
            except blockstore_api_exceptions.DraftHasNoChangesToCommit as exc:
                raise DraftHasNoChangesToCommit from exc
            except blockstore_api_exceptions.BundleStorageError as exc:
                raise BundleStorageError from exc
...
def _bundle_version_from_response(data):
    ...
    return BundleVersion(
        ...
        files={
            path: BundleFile(path=path, **filedata)
